...
class AsymmetricView extends StatelessWidget {
  ...
  AsymmetricView({Key key, this.category});
  ...
  List<Container> _buildColumns(BuildContext context) {
    ...
    return List.generate(_listItemCount(category.length), (int index) {
      double width = .59 * MediaQuery.of(context).size.width;
      Widget column;
      if (index % 2 == 0) {
        /// Even cases
        int bottom = _evenCasesIndex(index);
        // column = TwoCategoryCardColumn(
        //     bottom: category[bottom],
        //     top: category.length - 1 >= bottom + 1
        //         ? category[bottom + 1]
        //         : null);
        width += 32.0;
      } else {
        /// Odd cases
        column = OneCategoryCardColumn(
          // product: category[_oddCasesIndex(index)],
        );
      }
      return Container(
        width: width,
